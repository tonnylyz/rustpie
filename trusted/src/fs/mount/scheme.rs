...
unsafe impl<D: Disk> Sync for FileScheme<D> {}

/// SAFETY: only access in a thread
unsafe impl<D: Disk> Send for FileScheme<D> {}

pub struct FileScheme<D: Disk> {
  name: String,
  fs: RefCell<FileSystem<D>>,
  next_id: AtomicUsize,
  files: RefCell<BTreeMap<usize, Box<dyn Resource<D>>>>,
  ...
  fn resolve_symlink(
    ...
    for _ in 0..32 {
      ...
      let canon = canonicalize(
        &format!("{}{}", scheme, core::str::from_utf8(url).unwrap()).as_bytes(),
        &buf[0..count],
      );
      let path = core::str::from_utf8(&canon[scheme.len()..])
        .unwrap_or("")
        .trim_matches('/');
      nodes.clear();
