...
fn do_null() -> Result<(), &'static str> {
  ...
  info!("NULL result: ({})", T_UNIT);
  info!("{:?}", stats);
  info!("This test is equivalent to `lat_syscall null` in LMBench");
  Ok(())
}
...
fn do_null_inner(overhead_ct: u64, th: usize, nr: usize) -> Result<u64, &'static str> {
  ...
  info!("null_test_inner ({}/{}): hpet {} , overhead {}, {} total_time -> {} {} (ignore: {})",
...
fn do_spawn() -> Result<(), &'static str>{
  ...
  info!("SPAWN result: ({})", T_UNIT);
  info!("{:?}", stats);
  info!("This test is equivalent to `lat_proc exec` in LMBench");
...
fn do_spawn_inner(overhead_ct: u64, th: usize, nr: usize, _child_core: u8) -> Result<u64, &'static str> {
  ...
  info!("spawn_test_inner ({}/{}): hpet {} , overhead {}, {} total_time -> {} {}",
...
fn do_ctx() -> Result<(), &'static str> {
  ...
  let err = (lat * 10 * THRESHOLD_ERROR_RATIO) / 100;
  if 	max - lat > err || lat - min > err {
    warn!("ctx_test diff is too big: {} ({} - {}) {}", max-min, max, min, T_UNIT);
  }
  let stats = calculate_stats(&vec).ok_or("couldn't calculate stats")?;

  info!("Context switch result: ({})", T_UNIT);
  info!("{:?}", stats);
  info!("This test does not have an equivalent test in LMBench");
...
fn do_ctx_inner(th: usize, nr: usize, child_core: u8) -> Result<u64, &'static str> {
  ...
  info!("ctx_switch_test_inner ({}/{}): total_overhead -> {} {} , {} total_time -> {} {}",
...
fn do_memory_map() -> Result<(), &'static str> {
  ...
  let err = (lat * 10 * THRESHOLD_ERROR_RATIO) / 100;
  if 	max - lat > err || lat - min > err {
    warn!("memory_map_test diff is too big: {} ({} - {}) {}", max-min, max, min, T_UNIT);
  }
  let stats = calculate_stats(&vec).ok_or("couldn't calculate stats")?;

  info!("MEMORY MAP result: ({})", T_UNIT);
  info!("{:?}", stats);
  info!("This test is equivalent to `lat_mmap` in LMBench");
  Ok(())
}
...
fn do_memory_map_inner(overhead_ct: u64, th: usize, nr: usize) -> Result<u64, &'static str> {
  ...
  let delta_time = hpet_2_time("", delta_hpet);
  let delta_time_avg = delta_time / ITERATIONS as u64;
  info!("memory_map_test_inner ({}/{}): hpet {} , overhead {}, {} total_time -> {} {}",
		th, nr, delta_hpet, overhead_ct, delta_time, delta_time_avg, T_UNIT);
...
fn do_ipc_rendezvous(pinned: bool, cycles: bool) -> Result<(), &'static str> {
  ...
  let err = (lat * 10 * THRESHOLD_ERROR_RATIO) / 100;
  if 	max - lat > err || lat - min > err {
    warn!("ipc_rendezvous_test diff is too big: {} ({} - {})", max-min, max, min);
  }
  let stats = calculate_stats(&vec).ok_or("couldn't calculate stats")?;

  if cycles {
    info!("IPC RENDEZVOUS result: Round Trip Time: (cycles)",);
  } else {
    info!("IPC RENDEZVOUS result: Round Trip Time: ({})", T_UNIT);
  }
...
fn do_ipc_rendezvous_inner(th: usize, nr: usize, child_core: Option<u8>) -> Result<u64, &'static str> {
  ...
  let overhead_time = hpet_2_time("", delta_overhead);
  let delta_time_avg = delta_time / ITERATIONS as u64;
  info!("ipc_rendezvous_inner ({}/{}): total_overhead -> {} {} , {} total_time -> {} {}",
		th, nr, overhead_time, T_UNIT, delta_time, delta_time_avg, T_UNIT);
...
fn do_ipc_rendezvous_inner_cycles(th: usize, nr: usize, child_core: Option<u8>) -> Result<u64, &'static str> {
  ...
  info!("ipc_rendezvous_inner ({}/{}): total_overhead -> {} cycles , {} total_time -> {} cycles",
...
fn do_ipc_async(pinned: bool, blocking: bool, cycles: bool) -> Result<(), &'static str> {
  ...
  let err = (lat * 10 * THRESHOLD_ERROR_RATIO) / 100;
  if 	max - lat > err || lat - min > err {
    warn!("ipc_async_test diff is too big: {} ({} - {})", max-min, max, min);
  }
  let stats = calculate_stats(&vec).ok_or("couldn't calculate stats")?;

  if cycles {
    info!("IPC ASYNC result: Round Trip Time: (cycles)",);
  } else {
    info!("IPC ASYNC result: Round Trip Time: ({})", T_UNIT);
  }
  info!("{:?}", stats);
  info!("This test is equivalent to `lat_pipe` in LMBench when run with the pinned flag enabled");
...
fn do_ipc_simple(pinned: bool, cycles: bool) -> Result<(), &'static str> {
  ...
  let err = (lat * 10 * THRESHOLD_ERROR_RATIO) / 100;
  if 	max - lat > err || lat - min > err {
    warn!("ipc_simple_test diff is too big: {} ({} - {})", max-min, max, min);
  }
  let stats = calculate_stats(&vec).ok_or("couldn't calculate stats")?;

  if cycles {
    info!("IPC SIMPLE result: Round Trip Time: (cycles)");
  } else {
    info!("IPC SIMPLE result: Round Trip Time: ({})", T_UNIT);
  }
...
fn do_fs_read(with_open: bool) -> Result<(), &'static str>{
  let fsize_kb = 1024;
  info!("File size     : {} KB", fsize_kb);
  info!("Read buf size : {} KB", READ_BUF_SIZE / 1024);
  info!("========================================");
  ...
  if with_open {
    info!("This test is equivalent to `bw_file_rd open2close` in LMBench");
  } else {
    info!("This test is equivalent to `bw_file_rd io_only` in LMBench");
  }
  Ok(())
}
...
fn do_fs_read_with_size(overhead_ct: u64, fsize_kb: usize, with_open: bool) -> Result<(), &'static str> {
  ...
  if 	max - lat > err || lat - min > err {
    warn!("test diff is too big: {} ({} - {}) {}", max-min, max, min, T_UNIT);
  }

  print_header(TRIES, ITERATIONS);
  info!("{} for {} KB: {} {} {} MB/sec {} KB/sec",
...
fn do_fs_read_with_open_inner(filename: &str, overhead_ct: u64, th: usize, nr: usize) -> Result<(u64, u64, u64), &'static str> {
  ...
  info!("read_with_open_inner ({}/{}): {} total_time -> {} {} {} MB/sec {} KB/sec (ignore: {})",
...
fn do_fs_read_only_inner(filename: &str, overhead_ct: u64, th: usize, nr: usize) -> Result<(u64, u64, u64), &'static str> {
  ...
  info!("read_only_inner ({}/{}): {} total_time -> {} {} {} MB/sec {} KB/sec (ignore: {})",
		th, nr, delta_time, delta_time_avg, T_UNIT, mb_per_sec, kb_per_sec, _dummy_sum);
...
fn do_fs_create_del() -> Result<(), &'static str> {
  ...
  print_header(TRIES, ITERATIONS);
  info!("SIZE(KB)    Iteration    created(files/s)     time(ns/file)");
  for fsize_b in fsizes_b.iter() {
    do_fs_create_del_inner(*fsize_b, overhead_ct)?;
  }
  info!("This test is equivalent to file create in `lat_fs` in LMBench");
...
fn do_fs_create_del_inner(fsize_b: usize, overhead_ct: u64) -> Result<(), &'static str> {
  ...
  info!("{:8}    {:9}    {:16}    {:16}", fsize_b/KB as usize, ITERATIONS, files_per_time,delta_time_create / (ITERATIONS) as u64);
  Ok(())
}
...
fn do_fs_delete() -> Result<(), &'static str> {
  ...
  // info!("SIZE(KB)    Iteration    created(files/s)    deleted(files/s)");
  print_header(TRIES, ITERATIONS);
  info!("SIZE(KB)    Iteration    deleted(files/s)    time(ns/file)");
  for fsize_b in fsizes_b.iter() {
    do_fs_delete_inner(*fsize_b, overhead_ct)?;
  }
  info!("This test is equivalent to file delete in `lat_fs` in LMBench");
  Ok(())
}
...
fn do_fs_delete_inner(fsize_b: usize, overhead_ct: u64) -> Result<(), &'static str> {
  ...
  info!("{:8}    {:9}    {:16}    {:16}", fsize_b/KB as usize, ITERATIONS, files_per_time, delta_time_delete /(ITERATIONS) as u64);
  Ok(())
}
...
fn cat(fileref: &FileRef, sz: usize, msg: &str) {
  ...
  match file.read(&mut buf,0) {
    Ok(nr_read) => {
      info!("tries to read {} bytes, and {} bytes are read", sz, nr_read);
      info!("read: '{}'", str::from_utf8(&buf).unwrap());
    }
    Err(_) => {info!("Cannot read");}
  }
}
...
fn write(fileref: &FileRef, sz: usize, msg: &str) {
  ...
  match file.write(&buf,0) {
    Ok(nr_write) => {
      info!("tries to write {} bytes, and {} bytes are written", sz, nr_write);
      info!("written: '{}'", str::from_utf8(&buf).unwrap());
    }
    Err(_) => {info!("Cannot write");}
  }
}
