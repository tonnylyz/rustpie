...
pub fn page_fault_handler(va: usize) {
  ...
  if let Some(pte) = query(va) {
    ...
    let mut va_tmp = common::CONFIG_USER_STACK_TOP - 2 * PAGE_SIZE;
    loop {
...
pub fn set_page_fault_handler(handler: usize) {
  unsafe {
    if page_fault_handler_stub == 0 {
      mem_alloc(0, common::CONFIG_EXCEPTION_STACK_TOP - PAGE_SIZE, Entry::default());
      event_handler(0, asm_page_fault_handler as usize, common::CONFIG_EXCEPTION_STACK_TOP, 0);
    }
